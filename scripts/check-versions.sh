...
echo "package.json      .version           = ${NPM_VERSION:-<missing>}"
...
if [ "$PYPROJECT_VERSION" != "$INIT_VERSION" ] || [ "$PYPROJECT_VERSION" != "$NPM_VERSION" ]; then
  echo "::error::Version mismatch -- pyproject.toml=${PYPROJECT_VERSION} xrpl_camp/__init__.py=${INIT_VERSION} package.json=${NPM_VERSION}. All three must move together on every release."
  exit 1
fi
